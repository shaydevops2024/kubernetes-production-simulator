...
echo "📦 Checking prerequisites on your local machine..."
echo ""
...
echo "📤 Sending results to web app..."
RESPONSE=$(curl -X POST -H "Content-Type: application/json" -d "$JSON" "$REPORT_URL" -s)

if [ $? -eq 0 ]; then
    echo ""
    echo "✅ Check complete! Session ID: $SESSION_ID"
    echo "📊 View results in the web UI"
else
    echo ""
    echo "❌ Failed to send results to web app"
    ...
    try:
        ...
        if yaml_explanation_path.exists():
            try:
                with open(yaml_explanation_path, 'r', encoding='utf-8') as f:
                    scenario_info["yaml_explanation"] = f.read()
                    logger.info(f"Loaded yaml-explanation.md for ArgoCD scenario {scenario_id}")
            except Exception as e:
                logger.error(f"Error reading yaml-explanation.md for ArgoCD scenario {scenario_id}: {e}")
